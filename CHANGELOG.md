...
### Fixed
- Evaluate dynamic `import()` expressions through the configured `moduleResolver` instead of failing with `Unhandled node type: ImportExpression`.
- Preserve static import resolver behavior for dynamic imports, including importer `fromPath`, resolved-path module caching, native module exports, computed specifiers, and clear missing-module errors.

## [1.8.11] - 2026-05-08

### Fixed
- Resolve missing identifiers from a configured `globalThis` binding while preserving lexical scope precedence.
- Return original source text from interpreted function `toString()` without dropping runtime closure metadata.

## [1.8.10] - 2026-05-08

### Fixed
- Keep named and default import bindings live across circular module evaluation.
- Preserve constructable exported classes in circular TypeScript page-object imports.
...
### Fixed
- Support TypeScript `import x = require("module")` declarations through the configured module resolver.
- Share resolver caching and native module export handling between ESM imports and TypeScript import-equals declarations.
